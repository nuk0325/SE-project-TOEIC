...
from dialog.manager_unit_dialog import ManagerUnitDialog

class ManagerUnitWordNoteUI(QMainWindow) :
    def __init__(self, parent) :
        ...
        menuButton = QPushButton("메뉴", topFrame)
        menuButton.setFixedSize(QSize(60,60))
        menuButton.setStyleSheet("color : white;")
        menuButton.clicked.connect(lambda: self.closeAndOpen("menu"))

        topLayout = QHBoxLayout()
        topLayout.addWidget(backButton, alignment=Qt.AlignmentFlag.AlignLeft)
        topLayout.addWidget(noteLabel)
        topLayout.addWidget(menuButton, alignment=Qt.AlignmentFlag.AlignRight)
    ...
    def searchAndJudge(self) :
        if self.parent.searchAndAdd() == 0 :
            alarm = "유닛이 다 찼습니다"
            popup = ManagerUnitDialog(self, alarm, False)
            popup.exec()
        else :
    ...
    def deleteOrNot(self, frame, wordObj) :
        ...
        popup = ManagerUnitDialog(wordObj, alarm, True)
        popup.exec()
