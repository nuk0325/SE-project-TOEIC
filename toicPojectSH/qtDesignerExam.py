...
class Exam(QWidget, form_window):
    def __init__(self):
        ...
        # 주소 통일
        add_dic = {'강원도': '강원', '경기도': '경기', '경상남도': '경남', '경상북도': '경북', '광주광역시': '광주', '대구광역시': '대구',
                   '대전광역시': '대전',
                   '부산광역시': '부산', '서울시': '서울', '서울특별시': '서울', '서초구': '서울시 서초구', '수원시': '경기', '인천광역시': '인천',
                   '전라북도': '전북', '제주특별자치도': '제주', '충청북도': '충북'}
        ...
        for i in self.df_review.addresses:
            ...
            a = ' '.join(a)
            list.append(a)
        self.df_review.addresses = list
        print(self.df_review.addresses.head(100))
        self.df_review.to_csv('./model_data_Hospital_and_info2.csv')
    ...
    def cmb_title_slot(self):
        ...
        region = self.df_review[(self.df_review.category == title) & (self.df_review.region == address)].iloc[:10,
                 1]  # 자체 추천 순위로 출력
